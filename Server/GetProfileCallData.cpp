...
void GetProfileCallData::Proceed(bool ok) {
    if (status_ == CREATE) {
        ...
        std::string email = ctx_.auth_context()->FindPropertyValues("email").front().data();
        std::string pw = ctx_.auth_context()->FindPropertyValues("password").front().data();
        std::string username = ctx_.auth_context()->FindPropertyValues("username").front().data();
        std::string name = ctx_.auth_context()->FindPropertyValues("name").front().data();
        std::string surname = ctx_.auth_context()->FindPropertyValues("surname").front().data();
